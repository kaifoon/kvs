...
fn naive_thread_pool_spawn_counter() -> Result<()> {
    let logger = common::setup_logger();
    let pool = NaiveThreadPool::new(4, logger)?;
    spawn_counter(pool)
}
...
fn shared_queue_thread_pool_spawn_counter() -> Result<()> {
    let logger = common::setup_logger();
    let pool = SharedQueueThreadPool::new(4, logger)?;
    spawn_counter(pool)
}
...
fn rayon_thread_pool_spawn_counter() -> Result<()> {
    let logger = common::setup_logger();
    let pool = RayonThreadPool::new(4, logger)?;
    spawn_counter(pool)
}
